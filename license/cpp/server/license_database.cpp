...
namespace glasssix::license
{
	namespace
	{
		...
		constexpr std::string_view query_add_or_update_authorized_devices{ R"(INSERT INTO public.authorized_devices (license_id, machine_id, last_authorization_time) VALUES ($1, $2, TO_TIMESTAMP($3)) ON CONFLICT(machine_id) DO UPDATE SET license_id = $1, last_authorization_time = TO_TIMESTAMP($3))" };
	}
