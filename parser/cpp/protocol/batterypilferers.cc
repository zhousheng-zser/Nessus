...
#include <batterypilferers/box_info.hpp>
#include <batterypilferers/detect_code.hpp>

namespace glasssix::exposing::nessus::Protocol {

	class P_Batterypilferers : public Protocol
	{
		static Json::Value Batterypilferers_new_json(plugin_interface& plugin, Json::Value& root, param_span<std::uint8_t>& data, guid& instance, param_span<std::uint8_t>& external)
		{
			Json::Value value;
			...
			try {
				...
				instance = unbox<guid>(plugin.execute(u8"batterypilferers.new", param));
		...
		static Json::Value Batterypilferers_detect_json(plugin_interface& plugin, Json::Value& root, param_span<std::uint8_t>& data, guid& instance, param_span<std::uint8_t>& external)
		{
			Json::Value value;
			try
			{
				...
				auto result = plugin.execute(u8"batterypilferers.detect", param).as<exposing::param_vector<batterypilferers::box_info>>();
		...
		static Json::Value Batterypilferers_delete_json(plugin_interface& plugin, Json::Value& root, param_span<std::uint8_t>& data, guid& instance, param_span<std::uint8_t>& external)
		{
			Json::Value value;
			try
			{
				...
				plugin.execute(u8"batterypilferers.delete", param);
		...
		static Json::Value Batterypilferers_version_json(plugin_interface& plugin, Json::Value& root, param_span<std::uint8_t>& data, guid& instance, param_span<std::uint8_t>& external)
		{
			Json::Value value;
			try
			{
				...
				auto version = plugin.execute(u8"batterypilferers.version", param);
	...
	public:
		virtual const std::unordered_map<std::string, protocol_function> parser_protocol_dump() const override {
			...
			protocol_map["batterypilferers.new"] = &Batterypilferers_new_json;
			protocol_map["batterypilferers.delete"] = &Batterypilferers_delete_json;
			protocol_map["batterypilferers.detect"] = &Batterypilferers_detect_json;
			protocol_map["batterypilferers.version"] = &Batterypilferers_version_json;

			return protocol_map;
		}
	};

	REGISTE_PROTOCOL(P_Batterypilferers)
