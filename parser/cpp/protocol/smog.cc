...
//
#include <smog/detect_code.hpp>
#include <smog/box_info.hpp>

namespace glasssix::exposing::nessus::Protocol {

	class P_smog : public Protocol
	{
		static Json::Value Smog_new_json(plugin_interface& plugin, Json::Value& root, param_span<std::uint8_t>& data, guid& instance, param_span<std::uint8_t>& external)
		{
			Json::Value value;
			...
			try {
				...
				instance = unbox<guid>(plugin.execute(u8"smog.new", param));
		...
		static Json::Value Smog_version_json(plugin_interface& plugin, Json::Value& root, param_span<std::uint8_t>& data, guid& instance, param_span<std::uint8_t>& external)
		{
			Json::Value value;
			try
			{
				...
				auto version = plugin.execute(u8"smog.version", param);
		...
		static Json::Value Smog_detect_json(plugin_interface& plugin, Json::Value& root, param_span<std::uint8_t>& data, guid& instance, param_span<std::uint8_t>& external)
		{
			Json::Value value;
			try
			{
				...
				auto result = plugin.execute(u8"smog.detect", param).as<exposing::param_vector<smog::box_info>>();
		...
		static Json::Value Smog_delete_json(plugin_interface& plugin, Json::Value& root, param_span<std::uint8_t>& data, guid& instance, param_span<std::uint8_t>& external)
		{
			Json::Value value;
			try
			{
				...
				plugin.execute(u8"smog.delete", param);
	...
	public:
		virtual const std::unordered_map<std::string, protocol_function> parser_protocol_dump() const override {
			...
			protocol_map["smog.new"] = &Smog_new_json;
			protocol_map["smog.delete"] = &Smog_delete_json;
			protocol_map["smog.detect"] = &Smog_detect_json;
			protocol_map["smog.version"] = &Smog_version_json;

			return protocol_map;
		}
	};

	REGISTE_PROTOCOL(P_smog)
