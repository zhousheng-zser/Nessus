...
#include <pedestrian_mgg/classify_code.hpp>
...
using namespace glasssix::pedestrian_mgg;
...
namespace glasssix::exposing::nessus
{
	namespace
	{
		struct package_names
		{
			...
			static constexpr utf8_string_view pedestrian_mgg{ u8"pedestrian_mgg" };
		};

		struct function_names final
		{
			static constexpr utf8_string_view pedestrian_mgg_new{ u8"pedestrian_mgg.new" };
			static constexpr utf8_string_view pedestrian_mgg_detect{ u8"pedestrian_mgg.detect" };
			static constexpr utf8_string_view pedestrian_mgg_delete{ u8"pedestrian_mgg.delete" };
			...
			static constexpr utf8_string_view pedestrian_mgg_version{ u8"pedestrian_mgg.version" };
	...
	public:
		impl()
		{
			...
			functions_.insert_or_assign(function_names::pedestrian_mgg_new, std::bind(&impl::pedestrian_mgg_new, this, std::placeholders::_1));

			// Delete
			functions_.insert_or_assign(function_names::pedestrian_mgg_delete, meta::replace_return<unknown_object>(std::bind(&impl::delete_instance, this, std::placeholders::_1)));
			...
			// Business
			functions_.insert_or_assign(function_names::pedestrian_mgg_version, std::bind(&impl::pedestrian_mgg_version, this, std::placeholders::_1));
			...
			functions_.insert_or_assign(function_names::pedestrian_mgg_detect, std::bind(&impl::pedestrian_mgg_detect, this, std::placeholders::_1));
	...
	private:
		...
		unknown_object pedestrian_mgg_new(const param_hash_map<param_string, unknown_object>& params)
		{
			auto device = unbox<std::int32_t>(params.get_value(u8"device"));
			auto models_directory = unbox<param_string>(params.get_value(u8"models_directory"));
			return add_instance(package_names::playphone, make_exported_interface<glasssix::pedestrian_mgg::detect_code>(models_directory, device));
		}
		...
		unknown_object pedestrian_mgg_detect(const param_hash_map<param_string, unknown_object>& params)
		{
			constexpr std::int32_t channels = 3;
			auto instance = get_instance<pedestrian_mgg::detect_code>(params);
		...
		unknown_object pedestrian_mgg_version(const param_hash_map<param_string, unknown_object>& params)
		{
			auto instance = get_instance<pedestrian_mgg::detect_code>(params);
