...
namespace glasssix::exposing::nessus::Service
{
	class S_Workcloth :public Service { //Plugin
		...
		static unknown_object workcloth_detect(const param_hash_map<param_string, unknown_object>& params)
		{
			...
			auto color_hsv_cfg_abi = params.get_value(u8"color_hsv_cfg").as<exposing::param_hash_map<exposing::param_string, exposing::param_vector<int> > >();

			return instance.detect(image, channels, height, width, roi_x, roi_y, roi_width, roi_height, posture_info_list, params_map_abi, color_hsv_cfg_abi);
		}
