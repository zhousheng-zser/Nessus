...
namespace glasssix::exposing::nessus::Service
{
	class S_Onphone :public Service { //Plugin
		...
		static unknown_object onphone_detect2(const param_hash_map<param_string, unknown_object>& params)
		{
			constexpr std::int32_t channels = 3;
			auto instance = vision_service_impl::impl::get_instance<onphone::detect_code>(params);
			auto image = unbox<param_span<std::uint8_t>>(params.get_value(u8"image"));
			...
			return instance.exdetect(image, channels, height, width, roi_x, roi_y, roi_width, roi_height, posture_info_list, params_map_abi);
		}
		...
		static constexpr utf8_string_view detect2_{ u8"onphone.detect2" };
	public:
		virtual const void service_dump(std::unordered_map<param_string, service_function_ty>& service_map) const override {
			...
			service_map.try_emplace(detect2_, &onphone_detect2);
		}
	};
