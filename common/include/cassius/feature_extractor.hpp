...
namespace glasssix::exposing::impl
{
	template<> struct abi<cassius::feature_extractor>
	{
		...
		struct type : abi_unknown_object
		{
			...
			virtual std::int32_t G6_ABI_CALL init(abi_in_t<param_span<const param_string>> phai, abi_in_t<param_string> racy_path, std::int32_t device, bool use_int8) noexcept = 0;
	...
	struct interface_vtable<Derived, cassius::feature_extractor> : interface_vtable_base<Derived, cassius::feature_extractor>
	{
		...
		virtual std::int32_t G6_ABI_CALL init(abi_in_t<param_span<const param_string>> phai, abi_in_t<param_string> racy_path, std::int32_t device, bool use_int8) noexcept override
		{
			return abi_safe_call([&] { this->self().init(create_from_abi<param_span<const param_string>>(phai), create_from_abi<param_string>(racy_path), device, use_int8); });
		}
	...
	template<> struct abi_adapter<cassius::feature_extractor>
	{
		...
		struct type : enable_self_abi_awareness<Derived, cassius::feature_extractor>
		{
			...
			void init(param_span<const param_string> phai, const param_string& racy_path, std::int32_t device, bool use_int8) const
			{
				check_abi_result(this->self_abi().init(get_abi(phai), get_abi(racy_path), get_abi(device), get_abi(use_int8)));
			}
