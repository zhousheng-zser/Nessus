...
namespace glasssix::exposing::impl
{
	template<> struct abi<selene::feature_extractor>
	{
		...
		{
			virtual std::int32_t G6_ABI_CALL init(abi_in_t<param_string> racy_path, std::int32_t model_type, std::int32_t device, bool use_int8) noexcept = 0;
			virtual std::int32_t G6_ABI_CALL init(abi_in_t<param_span<const param_string>> phai, abi_in_t<param_string> racy_path, std::int32_t model_type, std::int32_t device) noexcept = 0;
	...
	{
		virtual std::int32_t G6_ABI_CALL init(abi_in_t<param_string> racy_path, std::int32_t model_type, std::int32_t device, bool use_int8) noexcept override
		{
			return abi_safe_call([&] { this->self().init(create_from_abi<param_string>(racy_path), model_type, device, use_int8); });
		}

		virtual std::int32_t G6_ABI_CALL init(abi_in_t<param_span<const param_string>> phai, abi_in_t<param_string> racy_path, std::int32_t model_type, std::int32_t device) noexcept override
		{
			return abi_safe_call([&] { this->self().init(create_from_abi<param_span<const param_string>>(phai), create_from_abi<param_string>(racy_path), model_type, device); });
		}
	...
	template<> struct abi_adapter<selene::feature_extractor>
	{
		...
		{
			void init(const param_string& racy_path, std::int32_t model_type, std::int32_t device, bool use_int8) const
			{
				check_abi_result(this->self_abi().init(get_abi(racy_path), get_abi(model_type), get_abi(device), get_abi(use_int8)));
			}

			void init(param_span<const param_string> phai, const param_string& racy_path, std::int32_t model_type, std::int32_t device) const
			{
				check_abi_result(this->self_abi().init(get_abi(phai), get_abi(racy_path), get_abi(model_type), get_abi(device)));
			}
